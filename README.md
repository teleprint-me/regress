...
Where:
- $SSE_{residual} = \sum{(y_{i} - \hat{y_{x_{i}}})}^2$, also called the **sum of squared errors (SSE)**, measures how far the actual values $y_i$ deviate from the predicted values $\hat{y}_{x_i}$ from the regression line.
